...
function result(
  ...
  const names = ['twitter', 'xiaohongshu', 'weixin'] as const;
  return {
...
describe('browser pipeline CLI exit codes', () => {
  it('returns 0 for success', async () => {
    ...
    const code = await runBrowserPipelineCli(['--dry-run'], {
      runPipeline: async () => result('success', ['success', 'success', 'success']),
  ...
  it('returns 0 and warns for partial success', async () => {
    ...
    const stderr = capture();
    const code = await runBrowserPipelineCli(['--dry-run'], {
      runPipeline: async () => result('partial_success', ['success', 'blocked', 'success']),
      stdout: stdout.writer,
      stderr: stderr.writer,
    });
    expect(code).toBe(0);
  ...
  it('returns 2 for a failed pipeline', async () => {
    ...
    const code = await runBrowserPipelineCli([], {
      runPipeline: async () => result('failed', ['unavailable', 'unavailable', 'unavailable']),
      stdout: stdout.writer,
      stderr: stderr.writer,
    });
    expect(code).toBe(2);
  ...
  it('keeps the complete JSON diagnostics on stdout when the pipeline fails', async () => {
    const stdout = capture();
    const stderr = capture();
    const failedResult = result('failed', ['login_required', 'blocked', 'unavailable']);
    const code = await runBrowserPipelineCli([], {
    ...
    expect(stderr.read()).toContain('twitter:login_required, xiaohongshu:blocked, weixin:unavailable');
  });
});

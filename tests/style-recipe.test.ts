...
describe('Style Recipe', () => {
  it('enforces owner >= 0.60, references <= 0.30 total, each <= 0.20, and at most two', () => {
    const recipe = buildStyleRecipe({ articleType: 'analysis', ownerProfile: owner, referenceProfiles: [firstReference, secondReference], referenceWeights: [0.15, 0.15] });
    ...
    expect(() => buildStyleRecipe({ articleType: 'analysis', ownerProfile: owner, referenceProfiles: [firstReference, secondReference, thirdReference] })).toThrow('at_most_two_reference_profiles');
    expect(() => buildStyleRecipe({ articleType: 'analysis', ownerProfile: owner, referenceProfiles: [firstReference], referenceWeights: [0.21] })).toThrow('reference_weight_limit_exceeded');
  ...
  it('is stable and reproducible for the same selected inputs', () => {
    const options = { articleType: 'tutorial' as const, ownerProfile: owner, referenceProfiles: [firstReference], referenceWeights: [0.2] };
  ...
  it('rejects an insufficient reference Profile', async () => {
    ...
    expect(() => buildStyleRecipe({ articleType: 'tutorial', ownerProfile: owner, referenceProfiles: [insufficient] })).toThrow('reference_profile_not_ready');
  });
